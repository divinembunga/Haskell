...
batch nm
  = do hreqs <- loadTheory [nm] hreqs0
       case currThry hreqs of
         Nothing    -> putStrLn ( "Failed to load data/"++nm++".thr")
         Just thry
...
showTFiles _ hreq
  = do listing <- getDirectoryContents "./data"
...
loadSource (fnroot:_) hreqs
  = do  mdl <- readHaskell fnroot
        ...
        let aststr = show mdl
        putStrLn aststr
        writeFile ("data/"++fnroot++".ast") aststr
        -- return $ hmods__ (++[mdl]) hreqs
        return hreqs

readHaskell fnroot
  = do let fname = fnroot ++ ".hs"
       modstr <- readFile ("data/"++fname)
